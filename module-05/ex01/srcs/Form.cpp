...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/27 08:56:05 by afogonca          #+#    #+#             */
/*   Updated: 2025/06/27 08:56:07 by afogonca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(void)
    : Name("Default"), IsSigned(false), ExecGrade(25), SignGrade(25) {
  std::cout << "[Form] Default constructor called" << std::endl;
}

Form::Form(const std::string _Name, const int _ExecGrade, const int _SignGrade)
    : Name(_Name), IsSigned(false), ExecGrade(_ExecGrade),
...
Form::Form(const Form &other)
    : Name(other.Name), IsSigned(other.IsSigned), ExecGrade(other.ExecGrade),
...
std::ostream &operator<<(std::ostream &os, const Form &obj) {
  os << "This form has the name " << obj.GetName() << " and currently it's";
  if (obj.GetIsSigned())
    os << " signed";
  else
    os << " not signed yet";
