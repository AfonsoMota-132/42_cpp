...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/27 08:57:35 by afogonca          #+#    #+#             */
/*   Updated: 2025/06/27 09:00:55 by afogonca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::SignForm(AForm &UnForm) {
  try {
    if (UnForm.BeSigned(this))
      std::cout << this->GetName() << " signed " << UnForm.GetName()
                << std::endl;
    else
      std::cout << this->GetName() << " couldn't sign " << UnForm.GetName()
...
void Bureaucrat::ExecuteForm(AForm &UnForm) {
  try {
    UnForm.Execute(this);
    std::cout << this->GetName() << " executed " << UnForm.GetName()
              << std::endl;
  } catch (std::exception &e) {
    std::cout << this->GetName() << " couldn't execute " << UnForm.GetName()
